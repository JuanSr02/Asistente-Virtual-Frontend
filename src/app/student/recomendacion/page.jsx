...
export default function Recomendacion({ user }) {
  ...
  useEffect(() => {
    if (state.persona && !state.historiaAcademica && state.planes.length === 0) {
      cargarPlanes()
    }
  ...
  const cargarDatosIniciales = async () => {
    ...
    try {
      ...
      let personaData = await personaService.obtenerPersonaPorSupabaseId(user.id)
      ...
      if (historia) {
        ...
        if (tieneRecomendacionesGuardadas) {
          ...
          console.log("No hay recomendaciones guardadas válidas, cargando nuevas")
          await obtenerRecomendaciones(personaData.id, state.criterioOrden, true)
        }
      } else {
  ...
  const obtenerRecomendaciones = async (estudianteId = state.persona?.id, orden = state.criterioOrden, isAutoLoad = false) => {
    ...
    try {
      console.log(`Obteniendo recomendaciones para estudiante ${estudianteId} con orden ${orden}`)
      const data = await recomendacionService.obtenerFinalesParaRendir(estudianteId, orden)
      ...
      if (err.response) {
        ...
        if (status === 404) {
          ...
          errorMessage = data?.message || "Datos inválidos para obtener recomendaciones."
        } else {
          errorMessage = data?.message || `Error ${status}: No se pudieron obtener las recomendaciones.`
        }
      }
  ...
  const handleFileUpload = async (event, isUpdate = false) => {
    ...
    const planAUsar = isUpdate ? state.historiaAcademica?.plan_de_estudio_codigo : state.planSeleccionado
    ...
    try {
      let resultado
      if (isUpdate) {
        resultado = await historiaAcademicaService.actualizarHistoriaAcademica(file, state.persona.id, planAUsar)
      } else {
        resultado = await historiaAcademicaService.cargarHistoriaAcademica(file, state.persona.id, planAUsar)
      }
      ...
      setTimeout(async () => {
        try {
          ...
          const historia = await historiaAcademicaService.verificarHistoriaAcademica(state.persona.id)
      ...
      if (err.response) {
        ...
        if (status === 400) {
          errorMessage = data?.message || "El archivo no tiene el formato correcto."
        } else if (status === 404) {
  ...
  const handleEliminarHistoria = async () => {
    ...
    try {
      await historiaAcademicaService.eliminarHistoriaAcademica(state.persona.id)
      updateState({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-purple-50 p-6 rounded-xl border">
        <h2 className="text-2xl font-bold text-gray-800 mb-2">📚 Recomendaciones Personalizadas</h2>
      ...
      {!state.historiaAcademica ? (
        <div className="bg-white p-8 rounded-lg shadow-md border border-orange-200">
          ...
          <div className="max-w-md mx-auto space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Selecciona tu plan de estudio:</label>
              <select
                value={state.planSeleccionado}
                onChange={(e) => updateState({ planSeleccionado: e.target.value })}
                disabled={state.loadingPlanes}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100"
              >
                <option value="">{state.loadingPlanes ? "Cargando planes..." : "Selecciona un plan"}</option>
            ...
            <div>
              ...
              <button
                ...
                className={`w-full py-3 px-4 rounded-lg font-semibold transition-all ${
                  state.uploading || !state.planSeleccionado || state.loadingPlanes
              ...
              <p className="text-xs text-gray-500 mt-2 text-center">Formatos permitidos: XLS, XLSX</p>
            </div>
          </div>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow-md border border-green-200">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-800">Historia académica ya cargada</h3>
          ...
          {state.loadingRecomendaciones ? (
            ...
            <div className="text-center py-12 bg-gray-50 rounded-xl">
              <div className="text-6xl mb-4">🎉</div>
              <h3 className="text-xl font-semibold text-gray-800 mb-2">¡No tienes finales para rendir!</h3>
              <p className="text-gray-600">
                Parece que ya has completado todos los finales disponibles según tu historia académica.
              </p>
            </div>
          ) : (
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                          <h4 className="text-xl font-semibold text-gray-800">{final.nombreMateria}</h4>
                          <p className="text-gray-600">Código: {final.codigoMateria}</p>
                        </div>
                      </div>
                    </div>

                    {state.criterioOrden === "ESTADISTICAS" && final.estadisticas && (
                      <span
                  ...
                  <div className="grid gap-4">
                    {state.criterioOrden === "CORRELATIVAS" && (
                      <div className="bg-blue-50 p-4 rounded-lg">
                        <p className="text-sm text-gray-600 mb-1">Importancia como correlativa:</p>
                    ...
                    {state.criterioOrden === "VENCIMIENTO" && (
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-green-50 p-4 rounded-lg">
                          <p className="text-sm text-gray-600 mb-1">Fecha de regularidad:</p>
                          <p className="font-semibold text-green-700">{formatearFecha(final.fechaRegularidad)}</p>
                        </div>
                        <div className="bg-orange-50 p-4 rounded-lg">
                          <p className="text-sm text-gray-600 mb-1">Fecha de vencimiento:</p>
                          <p className="font-semibold text-orange-700">{formatearFecha(final.fechaVencimiento)}</p>
                      ...
                      <div>
                        {final.estadisticas ? (
                          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                            ...
                            <div className="bg-blue-50 p-3 rounded-lg text-center">
                              ...
                              <p className="text-lg font-bold text-blue-700">
                                {(final.estadisticas.promedioNotas || 0).toFixed(1)}
                              </p>
                            </div>
                            <div className="bg-purple-50 p-3 rounded-lg text-center">
                              <p className="text-xs text-gray-600 mb-1">Días estudio</p>
                              <p className="text-lg font-bold text-purple-700">
                                {final.estadisticas.promedioDiasEstudio || 0}
                              </p>
                            </div>
                            <div className="bg-yellow-50 p-3 rounded-lg text-center">
                              <p className="text-xs text-gray-600 mb-1">Horas/día</p>
                            ...
                            <div className="bg-red-50 p-3 rounded-lg text-center">
                              ...
                              <p className="text-lg font-bold text-red-700">
                                {(final.estadisticas.promedioDificultad || 0).toFixed(1)}
                              </p>
                            </div>
                          </div>
                        ) : (
                          <div className="bg-gray-50 p-4 rounded-lg text-center">
                            ...
                            <p className="text-gray-600 font-medium">No hay estadísticas para esta materia</p>
                            <p className="text-sm text-gray-500 mt-1">
                              Esta materia aún no tiene suficientes datos estadísticos registrados
                            </p>
                          </div>
                        )}
                      </div>
