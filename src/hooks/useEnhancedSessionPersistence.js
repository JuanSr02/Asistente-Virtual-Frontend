...
export function useEnhancedSessionPersistence() {
    ...
    if (!initialized.current) {
      try {
        ...
        if (savedState) {
          ...
          const lastUpdate = parsed.lastUpdate ? new Date(parsed.lastUpdate) : null
          const now = new Date()
  ...
  const updateState = useCallback((updates) => {
    setState(prevState => {
      ...
      const significantKeys = ['persona', 'historiaAcademica', 'recomendaciones', 'planes', 'criterioOrden', 'planSeleccionado', 'personaId', 'lastFetch']
      const hasSignificantChanges = Object.keys(updates).some(key => significantKeys.includes(key))
  ...
  const isStateStale = useCallback((maxAgeMinutes = 30) => {
