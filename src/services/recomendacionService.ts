...
const recomendacionService = {
   ...
   */
  obtenerFinalesParaRendir: async (estudianteId: number, orden: OrdenFinales = "CORRELATIVAS"): Promise<FinalDTO[]> => {
    try {
      const response = await api.get(`/api/shared/finales/${estudianteId}`, { params: { orden } })
